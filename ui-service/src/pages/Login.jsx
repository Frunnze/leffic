...
export default function Login() {
    ...
    const [errLabel, setErrLabel] = createSignal("");

    async function loginUser() {
        // Call the login API with email and password
        if (!email() || !password()) {
            setErrLabel("Add your email and password")
            return;
        }
        ...
        if (res.status == 422) {
            setErrLabel("Invalid password or email");
            return
        }
        if (res.status == 404) {
            setErrLabel("Incorrect password or email");
            return;
        }
        setAccessToken(resData.access_token);
        if (res.ok) navigate("/folder/home");
    }

    return (
        <div class="bg-secondary/10 text-tertiary-100 w-screen h-screen flex justify-center items-center">
            <div class="shadow-xl border border-tertiary-10 rounded-md w-100 h-[70%] bg-primary p-8 flex flex-col justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 35 35" fill="none">
                ...
                <h2 class="text-secondary font-semibold text-xl text-center mb-6">Login</h2>
                ...
                <div class="w-full mb-6">
                    ...
                    />
                    <Show when={errLabel()}><span class="text-xs text-tertiary-red-80">{errLabel()}</span></Show>
                </div>
                ...
                <div class="relative w-full my-4 text-center">
                    <hr class="border-t border-tertiary-100/25" />
                    <span class="text-tertiary-100/60 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary px-2 text-sm text-tertiary">
                        or
                    </span>
                </div>
                <span class="hover:font-semibold text-sm cursor-pointer font-medium text-secondary" onClick={() => navigate("/sign-up")}>Sign up</span>
            </div>
        </div>
    );
}