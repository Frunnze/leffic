import { createResource, createSignal, createEffect, Switch, Match, onCleanup } from "solid-js";
...
const getFolders = async (folderId) => {
    ...
    if (folderId !== "home") {
        paramsToSend.append('folder_id', folderId);
    }
...
export default function Home() {
    ...
    createEffect(() => {
        const currentState = unitDropdownState();
        if (currentState) {
    ...
    return (
        <div class="flex text-tertiary-100">
            <LeftNavBar/>
            <div class="flex flex-col ml-17 pb-17 w-full h-full pt-18 px-15 md:px-40 lg:px-80 gap-y-4">
                ...
                <div class="flex flex-col gap-4 relative">
                    ...
                    <Show when={folderContent()} fallback={<p>Contents are loading...</p>}>
                        <For each={folderContent().content}>
                            {(unit) => (
                                <div class="relative">
                                    <A href={`/${unit.type}/` + unit.id} class="border flex justify-between gap-2 py-1.5 px-3 border-tertiary-40 rounded-md hover:bg-secondary hover:fill-primary hover:text-primary hover:border-transparent hover:shadow-sm cursor-pointer font-medium text-sm truncate">
