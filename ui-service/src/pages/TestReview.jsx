...
const getTest = async ({ testId, page = 1, per_page = 2 }) => {
  const res = await apiRequest({
    endpoint: `/api/content/test-items?${new URLSearchParams({
      test_id: testId,
      page,
      per_page
    }).toString()}`,
...
export default function TestReview() {
  ...
  const [correctAns, setCorrectAns] = createStore();

  onMount(async () => {
    const newTest = await getTest({
      testId: params.id,
      page: page()
    });
    setTest(newTest);
  ...
  const moveToNextItem = async () => {
    ...
    if (test.test_items.length === itemIndex()+1) {
      ...
      if (test.total_items === (test.page-1)*test.per_page+itemIndex()+1) {
        ...
        const res = await apiRequest({
          endpoint: `/api/content/test-session-results?test_session=${test.test_session}&test_id=${params.id}`,
        })
        if (res.ok) {
          const resData = await res.json()
          setCorrectAns(resData);
        }

        // Req to finish the test session
        await apiRequest({
          method: "POST",
          endpoint: `/api/content/end-test-session?test_session=${test.test_session}`,
        })
        ...
        const newTestPage = await getTest({testId: params.id, page: page() + 1});
        batch(() => {
          setItemIndex(0);
  ...
  const restartTest = async () => {
    ...
    localStorage.setItem(`testLastIndex${params.id}`, 0);
    const firstTestPage = await getTest({testId: params.id, page: 1});
    batch(() => {
      setItemIndex(0);
      setPage(1);
  ...
  return (
        <div class="text-tertiary-100 min-h-screen">
            <LeftNavBar/>
            <div class="min-h-screen ml-17 p-10 lg:px-30 bg-secondary/10 flex justify-center items-center">
                          ...
                          <div class="flex flex-col justify-content items-center gap-y-5">
                            <div class="text-center text-3xl">
                              ...
                              <span class="font-bold text-4xl">
                                {(correctAns.avg_accuracy*100).toFixed(1)}%
                              </span>
                              <br/>
                              <span class="text-2xl">
                                ({correctAns.correct}/{correctAns.total})
                              </span>
                            </div>
