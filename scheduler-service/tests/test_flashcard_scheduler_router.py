...
from src.features.flashcard_scheduling import flashcard_scheduler_router
...
def test_schedule_flashcard_with_a_stored_scheduler(
    ...
    stored: dict[str, object] = dict(Scheduler().to_dict())
    stored["_id"] = ObjectId()
    _ = _use_stored_scheduler(monkeypatch, stored)
...
def test_mongo_row2dict_drops_the_mongo_id() -> None:
    document: dict[str, object] = {
        "_id": ObjectId(),
        "user_id": _USER_ID,
    }

    plain = flashcard_scheduler_router.mongo_row2dict(document)

    assert plain == {"user_id": _USER_ID}
