...
def x_get_ratings_times__mutmut_10(
    ...
    for rating_value, rating in RATING_MAP.items():
        reviewed_card, _ = copy.deepcopy(restored_scheduler).review_card(
            copy.deepcopy(restored_card), None
        )
        seconds_until_due = (reviewed_card.due - timestamp).total_seconds()
        ratings_times[rating_value] = max(
            _NO_TIME_LEFT, int(seconds_until_due)
...
def x_get_ratings_times__mutmut_13(
    ...
    for rating_value, rating in RATING_MAP.items():
        reviewed_card, _ = copy.deepcopy(None).review_card(
            copy.deepcopy(restored_card), rating
        )
        seconds_until_due = (reviewed_card.due - timestamp).total_seconds()
        ratings_times[rating_value] = max(
...
def x_get_ratings_times__mutmut_14(
    ...
    for rating_value, rating in RATING_MAP.items():
        reviewed_card, _ = copy.copy(restored_scheduler).review_card(
            copy.deepcopy(restored_card), rating
        )
        seconds_until_due = (reviewed_card.due - timestamp).total_seconds()
        ratings_times[rating_value] = max(
...
def x_get_ratings_times__mutmut_16(
    ...
    for rating_value, rating in RATING_MAP.items():
        reviewed_card, _ = copy.deepcopy(restored_scheduler).review_card(
            copy.copy(restored_card), rating
        )
        seconds_until_due = (reviewed_card.due - timestamp).total_seconds()
        ratings_times[rating_value] = max(
            _NO_TIME_LEFT, int(seconds_until_due)
...
def x_get_ratings_times__mutmut_21(
    card: CardDict | None,
    scheduler: SchedulerDict | None,
) -> dict[int, int]:
    timestamp = datetime.now(UTC)
...
def x_get_ratings_times__mutmut_22(
    card: CardDict | None,
    scheduler: SchedulerDict | None,
) -> dict[int, int]:
    timestamp = datetime.now(UTC)
...
def x_get_ratings_times__mutmut_23(
    card: CardDict | None,
    scheduler: SchedulerDict | None,
) -> dict[int, int]:
    timestamp = datetime.now(UTC)
...
def x_get_ratings_times__mutmut_24(
    card: CardDict | None,
    scheduler: SchedulerDict | None,
) -> dict[int, int]:
    timestamp = datetime.now(UTC)
...
mutants_x_get_ratings_times__mutmut['x_get_ratings_times__mutmut_21'] = x_get_ratings_times__mutmut_21 # type: ignore # mutmut generated
mutants_x_get_ratings_times__mutmut['x_get_ratings_times__mutmut_22'] = x_get_ratings_times__mutmut_22 # type: ignore # mutmut generated
mutants_x_get_ratings_times__mutmut['x_get_ratings_times__mutmut_23'] = x_get_ratings_times__mutmut_23 # type: ignore # mutmut generated
mutants_x_get_ratings_times__mutmut['x_get_ratings_times__mutmut_24'] = x_get_ratings_times__mutmut_24 # type: ignore # mutmut generated
