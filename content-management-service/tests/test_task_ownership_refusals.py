...
from tests.task_token_support import NOT_FOUND, TASK_TOKENS
...
def test_a_swapped_folder_segment_is_refused() -> None:
    ...
    with pytest.raises(HTTPException) as refusal:
        _ = TASK_TOKENS.verified_task_id(swapped)
...
def test_a_token_signed_with_another_secret_is_refused(
    ...
    foreign = TASK_TOKENS.signed_task_id(_TASK_ID, _FOLDER_ID)
    monkeypatch.undo()

    with pytest.raises(HTTPException) as refusal:
        _ = TASK_TOKENS.verified_task_id(foreign)

    assert refusal.value.detail == MISSING_TASK


def test_a_different_folder_produces_a_different_digest() -> None:
    first_token = TASK_TOKENS.signed_task_id(_TASK_ID, _FOLDER_ID)
    second_token = TASK_TOKENS.signed_task_id(_TASK_ID, _OTHER_FOLDER_ID)
    first = first_token.split(".")[2]
    second = second_token.split(".")[2]
...
def test_a_segment_carrying_the_separator_is_refused(
    ...
    with pytest.raises(AmbiguousTokenSegmentError):
        _ = TASK_TOKENS.signed_task_id(task_id, folder_id)


def test_the_two_readings_of_one_token_can_no_longer_be_minted() -> None:
    with pytest.raises(AmbiguousTokenSegmentError):
        _ = TASK_TOKENS.signed_task_id("a", "b.c")
